...
class DashboardTests(unittest.TestCase):
    ...
    def test_dashboard_asset_has_no_uwp_controls(self):
        html = Path(__file__).resolve().parents[1].joinpath("assets", "dashboard.html").read_text(encoding="utf-8")

        self.assertNotIn("fixUwp", html)
        self.assertNotIn("fix-uwp", html)
        self.assertNotIn("UWP", html)
...
class DashboardServerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_stats_endpoint_serves_json(self):
        ...
        with patch("cleannet.dashboard.get_proxy_summary", return_value={
            "enabled": True,
            "owned_by_cleannet": True,
            "backend": "gnome",
            "supported": True,
            "error": "",
            "manual_env_file": "",
        ...
        self.assertIn(b'"proxy_backend": "gnome"', raw)
        self.assertIn(b'"example"', raw)
        self.assertTrue(writer.closed)

    async def test_fix_uwp_endpoint_is_not_registered(self):
        server, _state = self._server()
        writer = _Writer()

        await server.handle_http(_Reader(b"POST /api/fix-uwp HTTP/1.1\r\nHost: x\r\n\r\n"), writer)

        self.assertIn(b"404 Not Found", bytes(writer.data))
